...
import { useCreateSchedule } from "@/hooks/mutations/useScheduleMutations";
...
const AddScheduleDialog: React.FC<AddScheduleDialogProps> = ({
  ...
  const sanitizeDomList = useCallback(
    (arr: number[]) => {
      return [...new Set(arr.map(clampDom))].sort((a, b) => a - b);
    },
    [clampDom]
  );
  ...
  function validate(): boolean {
    ...
    if (!scheduleType.trim()) {
      showWarn(t("Please enter a type"));
      return false;
    }
    ...
    <>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button onClick={() => setIsOpenViewSchedules(true)} className="blue">
