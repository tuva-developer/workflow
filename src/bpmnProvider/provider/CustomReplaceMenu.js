...
export default function CustomReplaceMenu(popupMenu, replace, translate, modeling, bpmnFactory) {
...
CustomReplaceMenu.prototype.getEntries = function (element) {
    const {
        _replace,
        _translate,
        _modeling,
    ...
    if (is(element, 'bpmn:SubProcess') || is(element, 'bpmn:UserTask') || is(element, 'bpmn:ScriptTask')) {
        ...
        if (!isLoop || isSequential) {
            entries.push({
                ...
                action: {
                    click: () => {
                        const oldLoop = element.businessObject.loopCharacteristics;

                        let loop;
                        if (oldLoop && oldLoop.$type === 'bpmn:MultiInstanceLoopCharacteristics') {
                            loop = oldLoop;
                        } else {
                            loop = _bpmnFactory.create('bpmn:MultiInstanceLoopCharacteristics', {
                                isSequential: false
                            });
                        }
...
CustomReplaceMenu.prototype.getHeaderEntries = function () {
    return [];
};