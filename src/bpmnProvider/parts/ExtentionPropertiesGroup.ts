...
export function ExtentionPropertiesGroup({ element, injector }) {
  ...
  const items = extensionProperties.map((extensionProperty, index) => {
    ...
    return {
      ...
      remove: !IsReadOnlyMode
        ? removeExtensionProperty({ commandStack, element, extensionProperty })
        : undefined,
    };
  });
  ...
  return {
    items,
    add: !IsReadOnlyMode
      ? addExtensionProperty({ element, bpmnFactory, commandStack })
      : undefined,
  };
}

function ExtentionProperty(props) {
  const { idPrefix, extensionProperty } = props;
